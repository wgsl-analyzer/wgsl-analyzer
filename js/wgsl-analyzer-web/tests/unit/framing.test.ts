...
/** Asserts exactly one error was reported, and hands it back for matching. */
function onlyError(errors: readonly string[]): string {
	assert.equal(errors.length, 1, `expected exactly one error, got ${errors.length}`);
	const [first] = errors;
...
function frame(body: string, header?: string): Uint8Array {
...
describe("createFrameDecoder", () => {
	...
	it("reports a malformed body and recovers on the next frame", () => {
		...
		push(frame("not json"));
		push(frame('{"id":2}'));
		assert.match(onlyError(errors), /malformed JSON body/);
	...
	it("reports a header with no Content-Length", () => {
		...
		assert.match(onlyError(errors), /missing Content-Length/);
	});
	...
	it("reports an absurd Content-Length instead of stalling", () => {
		...
		assert.match(onlyError(errors), /exceeds the \d+ byte limit/);
		push(frame('{"id":1}'));
		assert.deepEqual(messages, [{ id: 1 }]);
	});
