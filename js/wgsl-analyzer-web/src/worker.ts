...
async function boot(message: Extract<HostMessage, { type: "boot" }>): Promise<void> {
	...
	// The server writes a raw byte stream, so this reassembles frames rather than
	// parsing each chunk.
	const pushChunk = createFrameDecoder(
		(lsp) => post({ type: "lsp", message: lsp }),
	...
	// Before `callMain`, which is when the server can first write. `pushChunk`
	// consumes synchronously, so the view into wasm memory never escapes.
	transport = module.lspStart({ onOutput: pushChunk });
