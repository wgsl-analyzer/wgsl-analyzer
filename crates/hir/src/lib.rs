        ...
        source.ancestors().find_map(|syntax| {
            match_ast! {
                match syntax {
                    ...
                    ast::GlobalVariableDeclaration(var) => self.global_variable_to_def(InFile::new(file_id, var)).map(DefWithBodyId::GlobalVariable),
                    ast::GlobalConstantDeclaration(constant) => self.global_constant_to_def(InFile::new(file_id, constant)).map(DefWithBodyId::GlobalConstant),
                    _ => None,
                }
            }
        })
    ...
    pub fn as_def_with_body_id(&self) -> Option<DefWithBodyId> {
        match *self {
            ...
            ModuleDef::Override(override_declaration) => {
                Some(DefWithBodyId::Override(override_declaration.id))
            },
