...
use syntax::{
    AstNode, AstPtr, SmolStr, SyntaxNode, SyntaxNodePointer, T, TextRange, ToSmolStr,
    ...
    pub fn diagnostics(
        ...
        for impl_def in self.impl_defs(db) {
            ...
            let drop_maybe_dangle = (|| {
                ...
                let trait_ = trait_?;
                let drop_trait = db
                    .lang_item(self.krate().into(), LangItem::Drop)?
                    .as_trait()?;
...
fn emit_def_diagnostic_(
    ...
    match diag {
        ...
        DefDiagnosticKind::UnconfiguredCode {
            tree,
            item,
            cfg,
            opts,
            ...
            (|| {
                let process_field_list =
                    |field_list: Option<_>, idx: ItemTreeFieldId| match field_list? {
                        ast::FieldList::RecordFieldList(it) => Some(SyntaxNodePointer::new(
                            it.fields()
                                .nth(idx.into_raw().into_u32() as usize)?
                                .syntax(),
                        )),
                        ast::FieldList::TupleFieldList(it) => Some(SyntaxNodePointer::new(
                            it.fields()
                                .nth(idx.into_raw().into_u32() as usize)?
                                .syntax(),
                        )),
                ...
                let ptr = match *item {
                    ...
                    AttrOwner::Field(FieldParent::Union(parent), idx) => SyntaxNodePointer::new(
                        ast_id_map
                            .get(item_tree[parent.index()].ast_id)
                            .to_node(&db.parse_or_expand(tree.file_id()))
                            .record_field_list()?
        ...
        DefDiagnosticKind::UnimplementedBuiltinMacro { ast } => {
            ...
            acc.push(
                UnimplementedBuiltinMacro {
                    node: ast.with_value(SyntaxNodePointer::from(AstPtr::new(&name))),
                }
                .into(),
            );
        },
        DefDiagnosticKind::InvalidDeriveTarget { ast, id } => {
            ...
            match derive {
                Some(derive) => {
                    acc.push(
                        InvalidDeriveTarget {
                            node: ast.with_value(SyntaxNodePointer::from(AstPtr::new(&derive))),
                        }
                        .into(),
                    );
                },
...
fn precise_macro_call_location(
    ast: &MacroCallKind,
    db: &dyn HirDatabase,
) -> (InFile<SyntaxNodePointer>, Option<TextRange>) {
    ...
    match ast {
        MacroCallKind::FnLike { ast_id, .. } => {
            ...
            (
                ast_id.with_value(SyntaxNodePointer::from(AstPtr::new(&node))),
                node.path()
        ...
        MacroCallKind::Attr {
            ...
            (
                ast_id.with_value(SyntaxNodePointer::from(AstPtr::new(&attr))),
...
impl Field {
    pub fn name(
        ...
        db.variant_fields(self.parent.into()).fields()[self.id]
            .name
            .clone()
    }
...
impl Variant {
    ...
    pub fn name(
        ...
        db.enum_variants(enum_).variants[lookup.index as usize]
            .1
            .clone()
    }
...
impl Adt {
    pub fn has_non_default_type_params(
        ...
        subst
            .iter()
            .any(|ty| match ty.skip_binders().data(Interner) {
        ...
        if let Ok(borrowck_results) = db.borrowck(self.into()) {
            for borrowck_result in borrowck_results.iter() {
                ...
                for moof in &borrowck_result.moved_out_of_ref {
                    let span: InFile<SyntaxNodePointer> = match moof.span {
                    ...
                    acc.push(
                        MovedOutOfRef {
                            ty: Type::new_for_crate(krate, moof.ty.clone()),
                            span,
                        }
                        .into(),
                    )
                ...
                for (binding_id, binding_data) in body.bindings.iter() {
                    ...
                    match (need_mut, is_mut) {
                        ...
                        (mir::MutabilityReason::Mut { spans }, false) => {
                            for span in spans {
                                let span: InFile<SyntaxNodePointer> = match span {
                                    mir::MirSpan::ExprId(e) => match source_map.expr_syntax(*e) {
                                        Ok(s) => s.map(|it| it.into()),
                                        Err(_) => continue,
                                    },
...
impl Function {
    ...
    pub fn returns_impl_future(
        ...
        let Some(impl_traits) = self.ret_type(db).as_impl_traits(db) else {
            return false;
        };
        let Some(future_trait_id) = db
            .lang_item(self.ty(db).env.krate, LangItem::Future)
            .and_then(|t| t.as_trait())
        else {
            return false;
        };
        let Some(sized_trait_id) = db
            .lang_item(self.ty(db).env.krate, LangItem::Sized)
            .and_then(|t| t.as_trait())
        else {
            return false;
        };
...
impl Const {
    ...
    pub fn eval(
        self,
        db: &dyn HirDatabase,
    ) -> Result<EvaluatedConst, ConstEvalError> {
...
impl LocalSource {
    ...
    pub fn syntax_ptr(self) -> InFile<SyntaxNodePointer> {
        self.source.map(|it| SyntaxNodePointer::new(it.syntax()))
    }
}
...
impl BuiltinAttr {
    ...
    fn builtin(name: &str) -> Option<Self> {
        hir_expand::inert_attr_macro::find_builtin_attr_idx(&Symbol::intern(name)).map(|idx| {
            BuiltinAttr {
                krate: None,
                idx: idx as u32,
            }
...
impl Impl {
    ...
    pub fn all_for_type(
        ...
        if let Some(block) = ty
            .adt_id(Interner)
            .and_then(|def| def.0.module(db).containing_block())
        {
    ...
    pub fn as_builtin_derive_path(
        ...
        let file_id = MacroFileId {
            macro_call_id: derive_attr,
        };
        let path = db
...
impl Type {
    ...
    pub fn future_output(
        ...
        let future_output = db
            .lang_item(self.env.krate, LangItem::FutureOutput)?
            .as_type_alias()?;
    ...
    pub fn iterator_item(
        ...
        let iterator_trait = db
            .lang_item(self.env.krate, LangItem::Iterator)?
            .as_trait()?;
    ...
    pub fn into_iterator_iter(
        ...
        let trait_ = db
            .lang_item(self.env.krate, LangItem::IntoIterIntoIter)
            .and_then(|it| {
    ...
    pub fn impls_trait(
        ...
        db.trait_solve(self.env.krate, self.env.block, goal)
            .is_some()
    }
...
impl Callable {
    ...
    pub fn receiver_param(
        ...
        Some((
            func.self_param(db)?,
            self.ty.derived(self.sig.params()[0].clone()),
        ))
    }
