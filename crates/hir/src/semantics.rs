...
use stdx::TupleExt;
use syntax::{
    AstNode, AstToken, Direction, SyntaxKind, SyntaxNode, SyntaxNodePointer, SyntaxToken,
    TextRange, TextSize,
...
impl<'db> SemanticsImpl<'db> {
    ...
    pub fn check_cfg_attr(
        ...
        let krate = match file_id.repr() {
            HirFileIdRepr::FileId(file_id) => {
                self.file_to_module_defs(file_id.file_id())
                    .next()?
                    .krate()
                    .id
            },
    ...
    pub fn as_format_args_parts(
        ...
        let token = self
            .wrap_token_infile(string.syntax().clone())
            .into_real_file()
            .ok()?;
    ...
    fn resolve_offset_in_format_args(
        ...
        if let Some(format_args) = ast::FormatArgsExpr::cast(parent.clone()) {
            ...
            let asm = self.wrap_node_infile(asm);
            source_analyzer
                .resolve_offset_in_asm_template(asm.as_ref(), line, offset)
    ...
    pub fn might_be_inside_macro_call(
        &self,
        token: &SyntaxToken,
    ) -> bool {
    ...
    fn descend_into_macros_impl<T>(
        ...
        let include = self
            .s2d_cache
            .borrow_mut()
            .get_or_insert_include_for(self.db, file_id);
        match include {
            Some(include) => {
    ...
    pub fn resolve_trait_impl_method(
        ...
        Some(
            self.db
                .lookup_impl_method(env.env, func.into(), substs.build())
                .0
                .into(),
        )
    }
    ...
    pub fn resolve_macro_call2(
        ...
        .or_else(|| {
            self.analyze(macro_call.value.syntax())?
                .resolve_macro_call(self.db, macro_call)
        })
    }
    ...
    pub fn is_inside_unsafe(
        ...
        loop {
            ...
            if let Some(parent) = ast::Expr::cast(parent.clone()) {
                if let Some(ExprOrPatId::ExprId(expr_id)) = source_map.node_expr(InFile {
                    file_id,
                    value: &parent,
                }) {
