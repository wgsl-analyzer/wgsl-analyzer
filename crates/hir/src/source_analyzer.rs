...
impl SourceAnalyzer {
    ...
    pub(crate) fn resolve_await_to_poll(
        ...
        let future_trait = db
            .lang_item(self.resolver.krate(), LangItem::Future)?
            .as_trait()?;
        let poll_fn = db
            .lang_item(self.resolver.krate(), LangItem::FuturePoll)?
            .as_function()?;
        // HACK: subst for `poll()` coincides with that for `Future` because `poll()` itself
        // doesn't have any generic parameters, so we skip building another subst for `poll()`.
        let substs = hir_ty::TyBuilder::subst_for_def(db, future_trait, None)
            .push(ty)
            .build();
    ...
    pub(crate) fn resolve_index_expr(
        ...
        let (index_trait, index_fn) = self.lang_trait_fn(
            db,
            LangItem::Index,
            &Name::new_symbol_root(sym::index.clone()),
        )?;
    ...
    pub(crate) fn resolve_try_expr(
        ...
        let op_fn = db
            .lang_item(self.resolver.krate(), LangItem::TryTraitBranch)?
            .as_function()?;
    ...
    pub(crate) fn resolve_path(
        ...
        let parent_hir_path = path
            .parent_path()
            .and_then(|p| collector.lower_path(p, &mut |_| TypeRef::Error));
    ...
    pub(crate) fn expand(
        ...
        self.store_sm()
            .and_then(|bs| bs.expansion(macro_call))
            .or_else(|| {
                self.resolver
                    .item_scope()
                    .macro_invoc(
    ...
    fn lang_trait_fn(
        ...
        let trait_id = db
            .lang_item(self.resolver.krate(), lang_trait)?
            .as_trait()?;
    ...
    fn ty_of_expr(
        ...
        self.infer()?
            .type_of_expr_or_pat(self.expr_id(expr.clone())?)
    }
}
...
fn type_of_expr_including_adjust(
    infer: &InferenceResult,
    id: ExprId,
) -> Option<&Ty> {
    match infer
