...
impl ChangeFixture {
    ...
    pub fn parse(wa_fixture: &str) -> Self {
        ...
        let current_source_root_kind = SourceRootKind::Local;
        ...
        //         crate_graph
        //             .add_dependency(from_id, Dependency::new(to.clone(), to_id))
        //             .unwrap();
        //     }
        // }
...
impl FileMeta {
    fn from_fixture(
        ...
        let introduce_new_source_root =
            fixture.introduce_new_source_root.map(|kind| match &*kind {
                "local" => SourceRootKind::Local,
                "library" => SourceRootKind::Library,
                invalid => panic!("invalid source root kind '{invalid}'"),
            });
        let _current_source_root_kind =
        ...
        assert!(
            introduce_new_source_root.is_none(),
            "Packages are not supported"
        );
        assert!(deps.is_empty(), "Packages are not supported");
        assert!(fixture.package.is_none(), "Packages are not supported");

        Self {
            path: fixture.path,
            // package: fixture
            //     .package
            //     .map(|it| parse_crate(it, current_source_root_kind, fixture.library)),
            // deps,
