...
//!
//! Normally, we get this "crate graph" by calling `wesl metadata --message-format=json`
...
//!   of `wesl-project.json` to rust-analyzer, no need to write anything to disk)
//!
...
impl ProjectJson {
    ...
    pub fn new(
        ...
        let sysroot_project =
            data.sysroot_project
                .zip(sysroot_src.clone())
                .map(|(sysroot_data, sysroot_src)| {
    ...
    pub fn crates(&self) -> impl Iterator<Item = (CrateArrayIdx, &Crate)> {
        self.crates
            .iter()
            .enumerate()
            .map(|(idx, krate)| (CrateArrayIdx(idx), krate))
    }
