...
//! in the salsa database -- `CrateGraph`.
...
use crate::{
    InvocationStrategy, ManifestPath, Package, ProjectJson, ProjectManifest, Sysroot, TargetData,
    TargetKind, WeslConfig, WeslSourceWorkspaceConfig, WeslWorkspace, WorkspaceBuildScripts,
    build_dependencies::BuildScriptOutput,
    project_json::{Crate, CrateArrayIdx},
    sysroot::RustLibSrcWorkspace,
    toolchain_info::{QueryConfig, version},
    utf8_stdout,
    wesl_workspace::{DepKind, PackageData, RustLibSource, WeslMetadataConfig},
};
...
pub enum ProjectWorkspaceKind {
    ...
    Wesl {
        ...
        rustc: Result<Box<(WeslWorkspace, WorkspaceBuildScripts)>, Option<String>>,
    },
...
impl ProjectWorkspace {
    ...
    fn load_wesl(
        ...
        let mut cmd = sysroot.tool(Tool::Wesl, workspace_dir, extra_env);
        cmd.args([
            "locate-project",
            "--workspace",
            "--manifest-path",
        ...
        tracing::info!(workspace = %wesl_toml, src_root = ?sysroot.rust_lib_src_root(), root = ?sysroot.root(), "Using sysroot");
        progress("querying project metadata".to_owned());
        let toolchain_config = QueryConfig::WeslRs(&sysroot, wesl_toml);
        let targets =
        ...
        let target_dir = config
            .target_dir
            .clone()
            .or_else(|| wesl_config_build_target_dir(wesl_toml, extra_env, &sysroot))
        ...
        let join = thread::scope(|s| {
            ...
            let data_layout = s.spawn(|| {
                target_data_layout::get(
                    ...
                    tracing::error!(%e, "failed fetching data layout for {wesl_toml:?} workspace")
                })
            });
            ...
            let rustc_dir = s.spawn(|| {
                ...
                rustc_dir.and_then(|rustc_dir| {
                    info!(workspace = %wesl_toml, rustc_dir = %rustc_dir, "Using rustc source");
                    match WeslWorkspace::fetch_metadata(
                        &rustc_dir,
                        workspace_dir,
                        &WeslMetadataConfig {
                            features: crate::WeslFeatures::default(),
                        ...
                        *no_deps,
                        ...
                        Ok((meta, _error)) => {
                            let workspace =
                                WeslWorkspace::new(meta, wesl_toml.clone(), Env::default(), false);
                        ...
                        Err(e) => {
                            tracing::error!(
                                %e,
                                "Failed to read wesl metadata from rustc source at {rustc_dir}",
                            );
                            Err(Some(format!(
                                "Failed to read wesl metadata from rustc source at {rustc_dir}: {e}"
                            )))
                        },
                    }
                })
            ...
            let loaded_sysroot = s.spawn(|| {
                sysroot.load_workspace(
                    &WeslSourceWorkspaceConfig::WeslMetadata(sysroot_metadata_config(
                        config,
                        &targets,
                        toolchain.clone(),
                        target_dir.clone(),
            ...
            let wesl_config_extra_env = s.spawn(|| wesl_config_env(wesl_toml, extra_env, &sysroot));
            thread::Result::Ok((
                rustc_cfg.join()?,
                data_layout.join()?,
                rustc_dir.join()?,
        ...
        if !wesl.requires_rustc_private() {
    ...
    pub fn load_inline(
        ...
        let target_dir = config
            ...
            .clone()
            .or_else(|| {
                wesl_config_build_target_dir(project_json.manifest()?, &config.extra_env, &sysroot)
            })
        ...
        let join = thread::scope(|s| {
            ...
            let loaded_sysroot = s.spawn(|| {
                if let Some(sysroot_project) = sysroot_project {
                    sysroot.load_workspace(
                        &WeslSourceWorkspaceConfig::Json(*sysroot_project),
                        config.no_deps,
                        project_root,
                        progress,
                    )
    ...
    pub fn load_detached_file(
        ...
        let query_config = QueryConfig::WeslRs(&sysroot, detached_file);
        let toolchain = version::get(query_config, &config.extra_env).ok().flatten();
        let targets = target_tuple::get(query_config, config.target.as_deref(), &config.extra_env)
            .unwrap_or_default();
        ...
        let target_dir = config
            .target_dir
            .clone()
            .or_else(|| wesl_config_build_target_dir(detached_file, &config.extra_env, &sysroot))
    ...
    pub fn run_build_scripts(
        ...
        match &self.kind {
            ...
            .with_context(|| format!("Failed to run build scripts for {}", wesl.workspace_root())),
    ...
    pub fn manifest(&self) -> Option<&ManifestPath> {
        match &self.kind {
            ...
            ProjectWorkspaceKind::DetachedFile { wesl, .. } => {
                Some(wesl.as_ref()?.0.manifest_path())
            },
        }
    }
            ...
            ProjectWorkspaceKind::Wesl {
                ...
                wesl.packages()
                    .map(|pkg| {
                        ...
                        let out_dir = build_scripts
                            .get_output(pkg)
                            .and_then(|it| it.out_dir.clone());
                        include.extend(out_dir);

                        // In case target's path is manually set in wesl.toml to be
                        ...
                        // path = "../../src/lib.rs"
                        // ```
                        let extra_targets = wesl[pkg]
                            .targets
                            .iter()
                            .filter(|&&tgt| matches!(wesl[tgt].kind, TargetKind::Lib { .. }))
                            .filter_map(|&tgt| wesl[tgt].root.parent())
    ...
    pub fn n_packages(&self) -> usize {
        ...
        match &self.kind {
            ...
            ProjectWorkspaceKind::Wesl { wesl, rustc, .. } => {
                let rustc_package_len = rustc
                    .as_ref()
                    .map(|a| a.as_ref())
                    .map_or(0, |(it, _)| it.packages().len());
                wesl.packages().len() + sysroot_package_len + rustc_package_len
            },
            ProjectWorkspaceKind::DetachedFile { .. } => sysroot_package_len,
        }
    }
    ...
    pub fn to_crate_graph(
        ...
        let (crate_graph, proc_macros) = match kind {
            ...
            ProjectWorkspaceKind::DetachedFile { file, .. } => detached_file_to_crate_graph(
                rustc_cfg.clone(),
                load,
                file,
                sysroot,
...
fn wesl_to_crate_graph(
    load: FileLoader<'_>,
    rustc: Option<&(WeslWorkspace, WorkspaceBuildScripts)>,
    wesl: &WeslWorkspace,
    sysroot: &Sysroot,
    rustc_cfg: Vec<CfgAtom>,
    override_cfg: &CfgOverrides,
    ...
    let _p = tracing::info_span!("wesl_to_crate_graph").entered();
    let mut res = (CrateGraphBuilder::default(), ProcMacroPaths::default());
    let (crate_graph, proc_macros) = &mut res;
    let (public_deps, libproc_macro) = sysroot_to_crate_graph(
        crate_graph,
    ...
    let workspace_proc_macro_cwd = Arc::new(wesl.workspace_root().to_path_buf());
    ...
    for pkg in wesl.packages() {
        let cfg_options = {
            ...
            if wesl[pkg].is_local {
                if set_test && !wesl.is_sysroot() {
                    // Add test cfg for local crates
                    cfg_options.insert_atom(sym::test);
                }
                cfg_options.insert_atom(sym::rust_analyzer);
            ...
            override_cfg.apply(&mut cfg_options, &wesl[pkg].name);
            cfg_options
        };

        let mut lib_tgt = None;
        for &tgt in wesl[pkg].targets.iter() {
            let pkg_data = &wesl[pkg];
            if !matches!(wesl[tgt].kind, TargetKind::Lib { .. })
                && (!pkg_data.is_member || wesl.is_sysroot())
            {
                // For non-workspace-members, WESL does not resolve dev-dependencies, so we don't
    ...
    for pkg in wesl.packages() {
        for dep in &wesl[pkg].dependencies {
            let Some(&to) = pkg_to_lib_crate.get(&dep.pkg) else {
                continue;
            };
            let Some(targets) = pkg_crates.get(&pkg) else {
                continue;
            };
    ...
    if wesl.requires_rustc_private() {
        ...
        if let Some((rustc_workspace, rustc_build_scripts)) = rustc {
            handle_rustc_crates(
                ...
                &cfg_options,
                override_cfg,
                // FIXME: Remove this once rustc switched over to wesl-project.json
                if rustc_workspace.workspace_root() == wesl.workspace_root() {
            ...
            for &tgt in pkg_data.targets.iter() {
                ...
                if let Some(file_id) = load(&rustc_workspace[tgt].root) {
                    let crate_id = add_target_crate_root(
                        ...
                        build_scripts
                            .get_output(pkg)
                            .zip(Some(build_scripts.error().is_some())),
                        cfg_options.clone(),
                        file_id,
...
fn add_target_crate_root(
    ...
    inject_wesl_package_env(&mut env, pkg);
    inject_wesl_env(&mut env);
    inject_rustc_tool_env(&mut env, wesl_name, kind);
    ...
    let crate_id = crate_graph.add_crate_root(
        file_id,
        edition,
        Some(CrateDisplayName::from_canonical_name(wesl_name)),
    ...
    } = kind
    {
        let proc_macro = match build_data {
            Some((
                ...
                Some(path) => Ok((wesl_name.to_owned(), path.clone())),
...
fn sysroot_to_crate_graph(
    ...
    match sysroot.workspace() {
        RustLibSrcWorkspace::Workspace(wesl) => {
            let (sysroot_cg, sysroot_pm) = wesl_to_crate_graph(
                load,
                None,
                wesl,
                &Sysroot::empty(),
        ...
        RustLibSrcWorkspace::Stitched(stitched) => {
            ...
            let libproc_macro = stitched
                .proc_macro()
                .and_then(|it| sysroot_crates.get(&it).copied());
