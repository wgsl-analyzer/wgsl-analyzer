...
//!
//! This crate lets a language server keep using [`lsp_server::Connection::stdio`], `std::io::stdin`
//! and `std::io::stdout` unchanged inside a Web Worker. Everything browser-specific happens below
//! Rust's `std::io` on libc level.
//!
//! The linker redirects four calls, `read`, `readv`, `write` and `writev`, so `read` reaches
//! `__wrap_read` while the original stays available as `__real_read`. Only fd 0 and fd 1 are
//! handled here. Everything else is forwarded untouched, so the filesystem behaves as emscripten
//! intends.
//!
//! ```text
...
//!
//! It asks for one byte at a time, up to whatever `len` the caller passed, and nothing tells it a
//! message has ended. A blocking implementation therefore parks on the byte *after* the last one
//! available, part way through a read, before a single byte reaches Rust.
//!
//! A non-blocking implementation is worse. Reporting "nothing yet" makes the loop return `i`, so an
//! empty queue at the start of a read returns zero, and zero from `read` is end of file. The
//! server's reader would shut down quietly rather than wait.
//!
//! `_wasmfs_stdin_get_char` is also not proxied, so it runs on whichever thread called `read`,
//! inside *that* worker's JavaScript context. Pthread workers start with no arguments, so the
//! `Module.stdin` installed on the main thread does not exist there at all.
//!
