...
impl AstIdMap {
    ...
    pub fn ast_id_for_ptr<N: AstIdNode>(
        &self,
        ptr: AstPointer<N>,
    ) -> FileAstId<N> {
        let ptr = ptr.syntax_node_pointer();
        let hash = hash_ptr(&ptr);
        match self
            .map
            .raw_entry()
    ...
    pub fn get<N: AstIdNode>(
        ...
        AstPointer::try_from_raw(self.arena[Idx::from_raw(RawIdx::from_u32(id.raw.into_raw()))])
            .unwrap()
    }
    ...
    fn erased_ast_id(
        ...
        let ptr = SyntaxNodePointer::new(item);
        let hash = hash_ptr(&ptr);
        match self
            .map
            .raw_entry()
    ...
    fn alloc(
        ...
        ErasedFileAstId(
            self.arena
                .alloc(SyntaxNodePointer::new(item))
                .into_raw()
                .into_u32(),
        )
    }
...
fn bdfs(
    node: &SyntaxNode,
    mut f: impl FnMut(SyntaxNode) -> TreeOrder,
) {
