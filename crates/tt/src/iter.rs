...
impl<'a, S: Copy> TtIter<'a, S> {
        ...
        match self.inner.as_slice().first()? {
            ...
            TokenTree::Subtree(subtree) => {
                let nested_iter = TtIter {
                    inner: self.inner.as_slice()[1..][..subtree.usize_len()].iter(),
                };
    ...
    pub fn from_savepoint(
        &self,
        savepoint: TtIterSavepoint<'a, S>,
    ) -> TokenTreesView<'a, S> {
...
impl<'a, S> Iterator for TtIter<'a, S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next()? {
            ...
            TokenTree::Subtree(subtree) => {
                let nested_iter = TtIter {
                    inner: self.inner.as_slice()[..subtree.usize_len()].iter(),
                };
