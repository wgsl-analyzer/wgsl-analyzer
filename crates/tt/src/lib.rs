...
impl<S: Copy> TopSubtreeBuilder<S> {
    ...
    pub fn extend(
        &mut self,
        leaves: impl IntoIterator<Item = Leaf<S>>,
    ) {
        self.token_trees
    ...
    pub fn extend_with_tt(
        &mut self,
        tt: TokenTreesView<'_, S>,
    ) {
    ...
    pub fn restore(
        &mut self,
        restore_point: SubtreeBuilderRestorePoint,
    ) {
        self.unclosed_subtree_indices
    ...
    S: Copy,
{
    ...
    let token = lexer::tokenize(text).next_tuple();
    let Some((lexer::Token {
        kind: lexer::TokenKind::Literal { kind, suffix_start },
        ..
    },)) = token
    else {
        return Literal {
...
impl<S> TopSubtree<S> {
    ...
    pub fn subtree_as_debug_string(
        &self,
        subtree_idx: usize,
    ) -> String {
        fn debug_subtree<S>(
