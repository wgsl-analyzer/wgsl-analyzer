/// Tests checking if attributes are handled in every position allowed by the WGSL grammar.
///
/// There is a test for every occurrence of `attribute` within the WGSL grammar, even
/// if some of those cases result in similar code paths of the formatter.
/// Every test is accompanied by a test that checks the placement of comments around
/// these attributes.
///
/// These tests only check if attributes are handled and the formatter doesn't crash.
/// They aren't meant to target order or exact formatting, (for those see [crate::tests::snapshots::attributes]
/// and [crate::tests::snapshots::attribute_ordering])
use expect_test::expect;

use crate::test_util::{check, check_comments};

//TODO(MonaMayrhofer) For now these tests just check for something so that attributes are handled and
// the formatter doesn't crash. More thought should be put into how exactly they shall be formatted

#[test]
pub fn format_attrs_on_struct_members() {
    check(
        "struct VertexOutput {
            @attr(0) @attr(1) position: vec4<f32>,
            @attr(0) @attr(1) uv: vec2<f32>,
        }",
        expect![[r#"
            struct VertexOutput {
                @attr(0)
                @attr(1)
                position: vec4<f32>,
                @attr(0)
                @attr(1)
                uv: vec2<f32>,
            }
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_struct_members() {
    check_comments(
        "struct VertexOutput { ##
        ## @attr(0) ## @attr(1) ## position ## : vec4<f32>,
        ## @attr(0) ## @attr(1) ## uv ## : vec2<f32>,
        }",
        expect![[r#"
            struct VertexOutput {
                /* 0 */ /* 1 */
                @attr(0) /* 2 */
                @attr(1) /* 3 */
                position: /* 4 */ vec4<f32>, /* 5 */
                @attr(0) /* 6 */
                @attr(1) /* 7 */
                uv: /* 8 */ vec2<f32>,
            }
        "#]],
        expect![[r#"
            struct VertexOutput {
                // 0
                // 1
                @attr(0) // 2
                @attr(1) // 3
                position: // 4
                vec4<f32>, // 5
                @attr(0) // 6
                @attr(1) // 7
                uv: // 8
                vec2<f32>,
            }
        "#]],
    );
}

#[test]
pub fn format_attrs_on_functions() {
    check(
        "@attr(0)
        @attr(1)
        fn main(
        ) {
        }",
        expect![[r#"
            @attr(0)
            @attr(1)
            fn main() {}
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_functions() {
    check_comments(
        "##@attr(0)##@attr(1)##fn##main(
        ) {
        }",
        expect![[r#"
            /* 0 */
            @attr(0) /* 1 */
            @attr(1) /* 2 */
            fn /* 3 */ main() {}
        "#]],
        expect![[r#"
            // 0
            @attr(0) // 1
            @attr(1) // 2
            fn // 3
            main() {}
        "#]],
    );
}

#[test]
pub fn format_attrs_on_function_return_type() {
    check(
        "
        fn thing(
        ) -> @attr(0) @attr(1) vec4<f32> {
        }",
        expect![[r#"
            fn thing() -> @attr(0)
            @attr(1)
            vec4<f32> {}
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_function_return_type() {
    check_comments(
        "
        fn thing(
        ) ## -> ## @attr(0) ## @attr(1) ## vec4<f32> ## {
        }",
        expect![[r#"
            fn thing() -> @location(0)
            vec4<f32> {}
        "#]],
        expect![[r#"
            fn thing() // 0
            -> @attr(0) // 2
            @attr(1) // 3
            // 1
            vec4<f32> // 4
            {}
        "#]],
    );
}

#[test]
pub fn format_attrs_on_function_parameter() {
    check(
        "
        fn thing(
            @attr(0) @attr(1) position: vec4<f32>,
            @attr(0) @attr(1) uv: vec2<f32>,
        ) -> vec4<f32> {
        }",
        expect![[r#"
            fn thing(
                @attr(0)
                @attr(1)
                position: vec4<f32>,
                @attr(0)
                @attr(1)
                uv: vec2<f32>,
            ) -> vec4<f32> {}
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_function_parameter() {
    check_comments(
        "
        fn thing ## ( ##
        ## @attr(0) ## @attr(1) ## position: vec4<f32>,
        ## @attr(0) ## @attr(1) ## uv: vec2<f32>,
        ) -> vec4<f32> {
        }",
        expect![[r#"
            fn thing /* 0 */ (
                /* 1 */ /* 2 */ @attr(0) /* 3 */
                @attr(1) /* 4 */
                position: vec4<f32>, /* 5 */
                @attr(0) /* 6 */
                @attr(1) /* 7 */
                uv: vec2<f32>,
            ) -> vec4<f32> {}
        "#]],
        expect![[r#"
            fn thing // 0
            (
                // 1
                // 2
                @attr(0) // 3
                @attr(1) // 4
                position: vec4<f32>, // 5
                @attr(0) // 6
                @attr(1) // 7
                uv: vec2<f32>,
            ) -> vec4<f32> {}
        "#]],
    );
}

#[test]
pub fn format_attrs_on_function_body() {
    check(
        "
        fn thing() -> vec4<f32> @attr(0) @attr(1) {
        }",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_function_body() {
    check_comments(
        "
        fn thing() -> ## vec4<f32> ## @attr(0) ## @attr(1) ## { ##
        }",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_global_variable() {
    check(
        "
        @attr(0) @attr(1) var<uniform> material: CustomMaterial;
        ",
        expect![[r#"
            @attr(0)
            @attr(1)
            var<uniform> material: CustomMaterial;
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_global_variable() {
    check_comments(
        "
        ## @attr(0) ## @attr(1) ## var<uniform> ## material: CustomMaterial;
        ",
        expect![[r#"
            /* 0 */
            @attr(0) /* 1 */
            @attr(1) /* 2 */
            var<uniform> /* 3 */ material: CustomMaterial;
        "#]],
        expect![[r#"
            // 0
            @attr(0) // 1
            @attr(1) // 2
            var<uniform> // 3
                material: CustomMaterial;
        "#]],
    );
}

#[test]
pub fn format_attrs_on_override() {
    check(
        "
        @attr(0) @attr(1) override amount: u64 = 0;
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_override() {
    check_comments(
        "
        ## @attr(0) ## @attr(1) ## override amount: u64 = 0;
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_compound_statement() {
    check(
        "
        fn main() {
            @attr(0) @attr(1) {}
            if true @attr(0) @attr(1) {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_compound_statement() {
    check_comments(
        "
        fn main() {
            ## @attr(0) ## @attr(1) ## { ## }
            ## if ## true ## @attr(0) ## @attr(1) ## { ## }
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_if_statement() {
    check(
        "
        fn main() {
            @attr(0) @attr(1) if true {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_if_statement() {
    check_comments(
        "
        fn main() {
            ## @attr(0) ## @attr(1) ## if true {}
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_switch_statement_and_body() {
    check(
        "
        fn main() {
            @attr(0) @attr(1) switch a @attr(0) @attr(1) {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_switch_statement_and_body() {
    check_comments(
        "
        fn main() {
            ## @attr(0) ## @attr(1) ## switch a ## @attr(0) ## @attr(1) ## { ## }
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_loop_statement_and_body() {
    check(
        "
        fn main() {
            @attr(0) @attr(1) loop @attr(0) @attr(1) {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_loop_statement_and_body() {
    check_comments(
        "
        fn main() {
            ## @attr(0) ## @attr(1) ## loop ## @attr(0) ## @attr(1) ## { ## }
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_for_statement() {
    check(
        "
        fn main() {
            @attr(0) @attr(1) for(var i = 0; i < 10; i++) {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_for_statement() {
    check_comments(
        "
        fn main() {
            ## @attr(0) ## @attr(1) ## for(var i = 0; i < 10; i++) {}
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_loop_continuing_block() {
    check(
        "
        fn main() {
        loop{
        continuing
        @attr(0) @attr(1) {}
        }
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_loop_continuing_block() {
    check_comments(
        "
        fn main() {
        loop{
        ## continuing ## @attr(0) ## @attr(1) ## { ## }
        }
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_attrs_on_while_statement() {
    check(
        "
        fn main() {
        @attr(0) @attr(1) while true {}
        }
        ",
        expect![[r#"
        "#]],
    );
}

#[test]
pub fn format_comments_in_attrs_on_while_statement() {
    check_comments(
        "
        fn main() {
        ## @attr(0) ## @attr(1) ## while ## true {}
        }
        ",
        expect![[r#"
        "#]],
        expect![[r#"
        "#]],
    );
}
