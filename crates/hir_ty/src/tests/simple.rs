...
fn address_of_not_reference() {
    check_infer(
        ...
        expect![[r#"
            ...
            42..44 '&x': [error]
            43..44 'x': i32
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(1), message: "cannot use unary operator `&` on type `i32`" } in Body
        "#]],
    );
}
...
fn vec_xy_is_not_ref() {
    check_infer(
        ...
        expect![[r#"
            ...
            46..50 'v.yx': vec2<i32>
            [EditionedFileId(Id(1800))] AssignmentNotAReference { left_side: Idx::<Expression>(4), actual: Type(2009) } in Body
        "#]],
    );
}
...
fn global_var_function_address_space_error() {
    check_infer(
        ...
        expect![[r#"
            ...
            [EditionedFileId(Id(1800))] UnexpectedTemplateArgument { expression: Idx::<Expression>(0) } in Signature
        "#]],
    );
}
...
fn no_builtin_overload() {
    check_infer(
        ...
        expect![[r#"
            ...
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(2), message: "cannot use binary operator `+` with operands `f32` and `mat2x2<f32>`" } in Body
        "#]],
    );
}
...
fn deref_not_a_pointer() {
    check_infer(
        ...
        expect![[r#"
            ...
            8..11 '*1f': [error]
            9..11 '1f': f32
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(0), message: "cannot use unary operator `*` on type `f32`" } in Body
        "#]],
    );
}
...
fn add_refs_and_ptrs() {
    check_infer(
        ...
        expect![[r#"
            ...
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(11), message: "cannot use binary operator `+` with operands `ptr<function, i32, read_write>` and `ptr<function, i32, read_write>`" } in Body
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(14), message: "cannot use binary operator `+` with operands `ptr<function, i32, read_write>` and `i32`" } in Body
        "#]],
    );
}
...
fn to_wgsl_types_builtin_struct() {
    check_infer(
        ...
        expect![[r#"
            ...
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(3), message: "`modf` expects a float scalar or vector argument" } in Body
        "#]],
    );
}
...
fn not_convertible() {
    check_infer(
        ...
        expect![[r#"
            ...
            23..32 '1i * 1.0f': [error]
            28..32 '1.0f': f32
            [EditionedFileId(Id(1800))] WgslError { expression: Idx::<Expression>(2), message: "cannot use binary operator `*` with operands `i32` and `f32`" } in Body
        "#]],
    );
}
