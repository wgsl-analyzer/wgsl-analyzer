...
enable f16;
fn foo() {
    let abstract_float = tan(1.0);
    let float_32 = tan(1.0f);
    let float_16 = tan(1.0h);

    let abstract_float_vec = tan(vec2(1.0));
    let float_32_vec = tan(vec2(1.0f));
    let float_16_vec = tan(vec2(1.0h));
}
",
        expect![[r#"
            31..45 'abstract_float': f32
            ...
            81..85 '1.0f': f32
            96..104 'float_16': f16
            107..116 'tan(1.0h)': f16
            111..115 '1.0h': f16
            127..145 'abstra...at_vec': vec2<f32>
            148..162 'tan(vec2(1.0))': vec2<float>
            152..161 'vec2(1.0)': vec2<float>
            157..160 '1.0': float
            172..184 'float_32_vec': vec2<f32>
            187..202 'tan(vec2(1.0f))': vec2<f32>
            191..201 'vec2(1.0f)': vec2<f32>
            196..200 '1.0f': f32
            212..224 'float_16_vec': vec2<f16>
            227..242 'tan(vec2(1.0h))': vec2<f16>
            231..241 'vec2(1.0h)': vec2<f16>
            236..240 '1.0h': f16
        "#]],
    );
...
enable f16;
fn foo() {
    let abstract_float = tanh(1.0);
    let float_32 = tanh(1.0f);
    let float_16 = tanh(1.0h);

    let abstract_float_vec = tanh(vec2(1.0));
    let float_32_vec = tanh(vec2(1.0f));
    let float_16_vec = tanh(vec2(1.0h));
}
",
        expect![[r#"
            31..45 'abstract_float': f32
            48..57 'tanh(1.0)': float
            53..56 '1.0': float
            67..75 'float_32': f32
            78..88 'tanh(1.0f)': f32
            83..87 '1.0f': f32
            98..106 'float_16': f16
            109..119 'tanh(1.0h)': f16
            114..118 '1.0h': f16
            130..148 'abstra...at_vec': vec2<f32>
            151..166 'tanh(vec2(1.0))': vec2<float>
            156..165 'vec2(1.0)': vec2<float>
            161..164 '1.0': float
            176..188 'float_32_vec': vec2<f32>
            191..207 'tanh(v...1.0f))': vec2<f32>
            196..206 'vec2(1.0f)': vec2<f32>
            201..205 '1.0f': f32
            217..229 'float_16_vec': vec2<f16>
            232..248 'tanh(v...1.0h))': vec2<f16>
            237..247 'vec2(1.0h)': vec2<f16>
            242..246 '1.0h': f16
        "#]],
    );
...
enable f16;
fn foo() {
    let abstract_float = trunc(1.0);
    let float_32 = trunc(1.0f);
    let float_16 = trunc(1.0h);

    let abstract_float_vec = trunc(vec2(1.0));
    let float_32_vec = trunc(vec2(1.0f));
    let float_16_vec = trunc(vec2(1.0h));
}
",
        expect![[r#"
            31..45 'abstract_float': f32
            48..58 'trunc(1.0)': float
            54..57 '1.0': float
            68..76 'float_32': f32
            79..90 'trunc(1.0f)': f32
            85..89 '1.0f': f32
            100..108 'float_16': f16
            111..122 'trunc(1.0h)': f16
            117..121 '1.0h': f16
            133..151 'abstra...at_vec': vec2<f32>
            154..170 'trunc(...(1.0))': vec2<float>
            160..169 'vec2(1.0)': vec2<float>
            165..168 '1.0': float
            180..192 'float_32_vec': vec2<f32>
            195..212 'trunc(...1.0f))': vec2<f32>
            201..211 'vec2(1.0f)': vec2<f32>
            206..210 '1.0f': f32
            222..234 'float_16_vec': vec2<f16>
            237..254 'trunc(...1.0h))': vec2<f16>
            243..253 'vec2(1.0h)': vec2<f16>
            248..252 '1.0h': f16
        "#]],
    );
