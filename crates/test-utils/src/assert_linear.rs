...
//!
//! See: <https://stats.stackexchange.com/questions/21844/selecting-best-model-based-on-linear-quadratic-and-cubic-fit-of-data>.
//!
...
impl AssertLinear {
    pub fn next_round(&mut self) -> bool {
        ...
        if self.rounds.iter().any(|item| item.linear) || self.rounds.len() == 4 {
            return false;
        }
...
impl Drop for AssertLinear {
    fn drop(&mut self) {
        ...
        if self.rounds.iter().all(|item| !item.linear) {
...
impl Round {
    fn finish(&mut self) {
        ...
        #[expect(clippy::as_conversions, reason = "required here")]
