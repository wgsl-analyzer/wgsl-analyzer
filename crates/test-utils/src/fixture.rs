...
impl MiniCore {
    ...
    #[track_caller]
    fn assert_valid_flag(
        &self,
        flag: &str,
    ) {
        if !self.valid_flags.iter().any(|it| it == flag) {
            panic!(
                "invalid flag: {flag:?}, valid flags: {:?}",
                self.valid_flags
            );
        }
    }
    ...
    pub fn source_code(mut self) -> String {
        ...
        let trim_doc: fn(&str) -> Option<&str> = |line| match line.strip_prefix("//!") {
            ...
            None => {
                assert!(
                    line.trim().is_empty(),
                    "expected empty line after minicore header"
                );
                None
            },
        };
...
"#,
    ...
    assert_eq!(
        mini_core.unwrap().activated_flags,
        vec!["coerce_unsized".to_owned()]
    );
