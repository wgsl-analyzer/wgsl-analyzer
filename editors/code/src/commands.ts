...
export function analyzerStatus(ctx: CtxInit): Cmd {
	const tdcp = new (class implements vscode.TextDocumentContentProvider {
		...
		async provideTextDocumentContent(_uri: vscode.Uri): Promise<string> {
			...
			return await client.sendRequest(wa.analyzerStatus, params);
		}
...
export function viewFileText(ctx: CtxInit): Cmd {
	const tdcp = new (class implements vscode.TextDocumentContentProvider {
		...
		async provideTextDocumentContent(
			...
			return client.sendRequest(wa.viewFileText, params, ct);
		}
...
export function viewItemTree(ctx: CtxInit): Cmd {
	const tdcp = new (class implements vscode.TextDocumentContentProvider {
		...
		async provideTextDocumentContent(
			...
			return client.sendRequest(wa.viewItemTree, params, ct);
		}
...
function crateGraph(ctx: CtxInit, full: boolean): Cmd {
	return async () => {
		...
		const client = ctx.client;
		const dot = await client.sendRequest(wa.viewCrateGraph, params);
...
export function runFlycheck(ctx: CtxInit): Cmd {
	return async () => {
		...
		await client.sendNotification(wa.runFlycheck, { textDocument: params });
	};
}
