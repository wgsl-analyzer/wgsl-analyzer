...
async function resolveImport(content: string): Promise<string> {
	...
	let uri = vscode.Uri.parse(content_replaced);
	...
	private async getOrCreateClient() {
		...
		if (!this._client) {
			...
			this.pushClientCleanup(
				this._client.onNotification(wa.serverStatus, (params) =>
	...
	async start() {
		...
		this.subscriptions.push(
			client.onRequest(wa.requestConfiguration, async (_, __) => {
				let options = await lspOptions(this.config);
				return options;
			}),
			client.onRequest(wa.importTextDocument, async (params, __) => {
	...
	private updateStatusBarItem() {
		...
		statusBar.tooltip.appendMarkdown(
			`[Extension Info](command:wgsl-analyzer.serverVersion "Show version and server binary info"): Version ${this.version}, Server Version ${this._serverVersion}` +
				...
				"\n\n" +
				'[$(refresh) Reload Workspace](command:wgsl-analyzer.reloadWorkspace "Reload and rediscover workspaces")' +
				"\n\n" +
				'[$(symbol-property) Rebuild Build Dependencies](command:wgsl-analyzer.rebuildProcMacros "Rebuild build scripts and proc-macros")' +
				"\n\n" +
