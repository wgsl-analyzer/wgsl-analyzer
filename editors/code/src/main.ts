...
If you are using a version of wgsl-analyzer without a prepackaged binary or specify a custom server path, please use the matching version: \`cargo install --git https://github.com/wgsl-analyzer/wgsl-analyzer --rev ${REV} wgsl-analyzer\`.`;
    ...
    vscode.workspace.onDidChangeConfiguration(
        (_) =>
            ctx.client.sendNotification("workspace/didChangeConfiguration", {
                settings: "",
            }),
        null,
        ctx.subscriptions,
...
async function getServer(config: Config): Promise<string | undefined> {
    ...
    const bundledPath = path.resolve(__dirname, `wgsl-analyzer${suffix}`);
    ...
    vscode.window.showErrorMessage(
        "wgsl-analyzer.server.path is not specified",
    );
    return undefined;
}
