...
import { sendRequestWithRetry, isWgslDocument } from './util';
...
export function activateInlayHints(ctx: Ctx) {
    const maybeUpdater = {
        ...
        async onConfigChange() {
            ...
            this.hintsProvider = vscode.languages.registerInlayHintsProvider({ language: "wgsl" }, new class implements vscode.InlayHintsProvider {
        ...
        onDidChangeTextDocument({ contentChanges, document }: vscode.TextDocumentChangeEvent) {
            if (contentChanges.length === 0 || !isWgslDocument(document)) return;
